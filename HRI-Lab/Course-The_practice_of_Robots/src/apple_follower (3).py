...
from geometry_msgs.msg import PointStamped, Vector3, Point, Quaternion, Twist
...
def talker():
    global pub, apple_counter, apple_position, nav_state, scan_position_flug, apple_state, angle_min, angle_max, angle_increment, ranges, _odom_x_, _odom_y_, _odom_theta_
    ...
    while np.count_nonzero(apple_counter) < 5 and not rospy.is_shutdown():
        for i in range(len(nav_position)):
            des_x = nav_position[i][0]
            des_y = nav_position[i][1]
            ...
            elif(scan_position_flug[nav_state]):
                if(nav_state == 0): #自分の位置からリンゴポジ[0]までの距離とセンサーから取得した距離がほぼ一致ならリンゴありと認定
                    now_position_x = nav_position[i][0]
                    now_position_y = nav_position[i][1]
                    des_x = apple_position[len(apple_counter)][0]
                    des_y = apple_position[len(apple_counter)][1]
                    markers = MarkerArray()
                    for i in range(len(ranges)):
                        ...
                        a = np.array([_odom_x_, _odom_y_])
                        b = np.array([des_x, des_y]) - a
                        cos_theta =  np.dot(a, b) / (np.linalg.norm(a) * np.linalg.norm(b))
                        apple_theta = np.arccos(cos_theta) * 180 / np.pi
                        theta_diff = apple_theta - theta

                        if(-THETA_ERROR_RANGE <= theta_diff and theta_diff <= THETA_ERROR_RANGE):
                            c = np.array([des_x, des_y])
                            theoretical_distance = np.linalg.norm(a-c) #theoretical_distance
                            d = np.array([x, y])
                            actual_distance = np.linalg.norm(a-d)
                            if(abs(theoretical_distance-actual_distance) <= 0.3):
                                move_to(des_x, des_y, rate)
                                move_to(now_position_x, now_position_y, rate)
                                apple_counter.append(True)
                            ###
