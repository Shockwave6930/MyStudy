...
class SphericalGrasps(object):
    ...
    def dyn_rec_callback(self, config, level):   #このコールバック関数はユーザーがdyn_rec_srvの変数の変更を適用する関数
        ...
        self._gripper_pre_grasp_positions = config["gripper_pre_grasp_positions"]   #Graspに入る前のエンドエフェクタグループのジョイントの軌道位置を定義
    ...
    def create_grasps_from_object_pose(self, object_pose):   #pick_and_place_server.py内で二回呼ばれるspherical_grasps_server.pyの1つの関数
        """
        :type object_pose: PoseStamped
        """
        tini = rospy.Time.now()
        sphere_poses = self.generate_grasp_poses(object_pose)
        filtered_poses = filter_poses(sphere_poses, object_pose, filter_behind=False, filter_under=True)
        sorted_poses = sort_by_height(filtered_poses)
        grasps = self.create_grasps_from_poses(sorted_poses)
        tend = rospy.Time.now()
        rospy.loginfo("Generated " + str(len(grasps)) + " grasps in " + str((tend - tini).to_sec()))   #二つのrospy.Time()の差分時間で何個のgraspsが作成されたかをログ出力
    ...
    def create_placings_from_object_pose(self, posestamped):   #pick_and_place_server.py内で二回呼ばれるspherical_grasps_server.pyの1つの関数
