...
class PickAruco(object):
	...
	def pick_aruco(self, string_operation):
		while not rospy.is_shutdown():
			rospy.loginfo("===================================================READY FOR NEXT EXPERIMENT?:===================================================")
			check1 = input()
			if(check1 == 1):
				rospy.loginfo("Done Checking 1!")
			if(self.i == 0):
				self.prepare_robot()
			else:
				pmg = PlayMotionGoal()
				pmg.motion_name = 'pregrasp_2' #revised(dafault="pregrasp")
				pmg.skip_planning = False
				self.play_m_as.send_goal_and_wait(pmg)
				self.lower_head()
				rospy.loginfo("Robot prepared_2.")
			rospy.loginfo("===================================================READY FOR PICK?:===================================================")
			check2 = input()
			#print("input:{0}".format(check))
			if(check2 == 1):
				rospy.loginfo("Done Checking 2!")
			rospy.sleep(2.0)
			rospy.loginfo("spherical_grasp_gui: Waiting for an aruco detection")
			rospy.loginfo("Sleeping in 10 seconds!")
			rospy.sleep(10.0)

			aruco_pose = rospy.wait_for_message('/aruco_single/pose', PoseStamped)   #pose
			...
			while not transform_ok and not rospy.is_shutdown():
				try:
					transform = self.tfBuffer.lookup_transform("base_footprint", 
										ps.header.frame_id,
										rospy.Time(0))
					aruco_ps = do_transform_pose(ps, transform)   #pose
					transform_ok = True
			...
			if string_operation == "pick":
				...
				#pick_g.object_pose.pose.orientation.w = 1.0  #revised
                                pick_g.object_pose.pose.orientation = aruco_ps.pose.orientation
                                print("\n\n\n\n\n=====================DEBUG==================")
                                print("pick_client.py: ", pick_g.object_pose.pose)
                                print("=====================DEBUG==================\n\n\n\n\n")
				self.detected_pose_pub.publish(pick_g.object_pose)   #pose
				...
				if str(moveit_error_dict[result.error_code]) != "SUCCESS":
					...
					rospy.loginfo("Let's go next repetition!")
				self.i += 1
				# Raise arm
