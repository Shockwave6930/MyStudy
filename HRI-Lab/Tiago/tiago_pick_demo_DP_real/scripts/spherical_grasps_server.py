...
class SphericalGrasps(object):
    ...
    def create_grasp(self, pose, grasp_id):
        ...
        grasp_posture = copy.deepcopy(pre_grasp_posture)   #全く同じpostureなのはこの姿勢で1秒間待機>させるため
        ...
        g.grasp_pose = PoseStamped(header, fixed_pose)   #Grasp.grasp_poseはgraspのためのエンドエフェクタの位置を示すが、内容はエンドエフェクタのparentのリンクのポーズであり、実際にはエンドエフェクタ内>のリンクではない。一般的にはエンドエフェクタが始まる前の最も遠い位置の手首のリンクのポーズになる
        ...
        #g.pre_grasp_approach.direction.header.frame_id = "base_footprint" #base_footprintに設定する>と、本当にその座標系でのx軸方向から物体に対して侵入を行う。ただし本デモではグリッパーのコリジョンが無視されているので、Rviz上では物体をアームが通過する
    ...
    def create_grasps_from_object_pose(self, object_pose):
        ...
        rospy.loginfo("Generated " + str(len(grasps)) +
                      " grasps in " + str((tend - tini).to_sec()))   #二つのrospy.Time()の差分時間で>何個のgraspsが作成されたかをログ出力
